...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--domain',
            type=str,
            help='Domínio específico (obrigatório para generate, check, delete)'
        )
        parser.add_argument(
            '--email',
            type=str,
            help='Email para notificações do Let\'s Encrypt'
        )
    ...
    def generate_certificate(self, domain, email):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f'⚠️  Landing page não encontrada para {domain}. Continuando...'
                )
            )
    ...
    def renew_all_certificates(self):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'✅ Renovação concluída: {renewed} renovados, {errors} erros'
            )
        )
    ...
    def check_certificate(self, domain):
        ...
        if info.get('has_certificate'):
            self.stdout.write(self.style.SUCCESS(f'✅ Certificado encontrado'))
            if 'expiry_date' in info:
                self.stdout.write(f'📅 Expira em: {info["expiry_date"]}')
        else:
            self.stdout.write(self.style.WARNING(f'⚠️  Sem certificado'))
            if 'error' in info:
    ...
    def list_certificates(self):
        ...
        for lp in landing_pages:
            ...
            if lp.ssl_error:
                self.stdout.write(
                    self.style.ERROR(f'   └─ Erro: {lp.ssl_error[:100]}...')
                )
